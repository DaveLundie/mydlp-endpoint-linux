...
from mydlpfuse import FUSE, FuseOSError, Operations, LoggingMixIn
...
class MyDLPFilter(LoggingMixIn, Operations):
    ...
    def fsync(self, path, datasync, fh):
        print "FSYNC is called"
        return 0;

    def release(self, path, fh):
        time_now = strftime("%Y-%m-%d %H:%M:%S", gmtime())
        print time_now + " -> RELEASE is called"
        return 0
    ...
    def write(self, path, data, offset, fh):
        print "WRITE Request is started"
...
def start_fuse(mount_point, safe_point):
    ...
    fuse = FUSE(MyDLPFilter(), mount_point, foreground=True, nonempty=True, 
                allow_other=True)
...
if __name__ == "__main__":
    if len(argv) != 2:
        ...
        exit(1)
    #fuse = FUSE(Memory(), argv[1], foreground=True, nonempty=True)
    mount_point = argv[1]
